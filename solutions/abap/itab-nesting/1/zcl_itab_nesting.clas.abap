...
CLASS zcl_itab_nesting IMPLEMENTATION.
  ...
  METHOD perform_nesting.
    LOOP AT artists ASSIGNING FIELD-SYMBOL(<fs_artist>).
      APPEND VALUE #(
        artist_id = <fs_artist>-artist_id
        artist_name = <fs_artist>-artist_name
        albums = VALUE #(
          FOR album in albums
          WHERE ( artist_id = <fs_artist>-artist_id )
          (
            album_id = album-album_id
            album_name = album-album_name
            songs = VALUE #(
              FOR song IN songs
              WHERE ( artist_id = <fs_artist>-artist_id AND album_id = album-album_id )
